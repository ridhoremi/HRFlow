...
class ZKLibrary
{
    ...
    public function getAttendance()
    {
        ...
        $u = unpack(
            'H2h1/H2h2/H2h3/H2h4/H2h5/H2h6/H2h7/H2h8',
            substr($this->received_data, 0, 8)
        );
        ...
        try {
            ...
            if ($bytes) {
                ...
                while ($bytes > 0 && $counter < 1000) {
                    ...
                    if ($received_data != '') {
                        $this->attendance_data[] = $received_data;
                    }
            ...
            if (count($this->attendance_data) > 0) {
                ...
                for ($x = 0; $x < count($this->attendance_data); $x++) {
                    ...
                    if ($x > 0) {
                        $this->attendance_data[$x] =
                            substr($this->attendance_data[$x], 8);
                    }
                }
                ...
                while (strlen($attendance_data) > 40) {
                    ...
                    $id = str_replace(
                        "\0",
                        '',
                        hex2bin(substr($u[1], 8, 16))
                    );

                    $state = hexdec(substr($u[1], 56, 2));

                    $timestamp = $this->decodeTime(
                        hexdec(
                            $this->reverseHex(substr($u[1], 58, 8))
                        )
                    );
